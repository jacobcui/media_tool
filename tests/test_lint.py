...
def test_black_formatting():
    ...
    result = subprocess.run(
        ["black", ".", "media_tool"], capture_output=True, text=True
    )
    assert (
        result.returncode == 0
...
def test_flake8_compliance():
    ...
    result = subprocess.run(
        ["flake8", ".", "media_tool"], capture_output=True, text=True
    )
    assert (
        result.returncode == 0
...
def test_isort_check():
    ...
    result = subprocess.run(
        ["isort", ".", "media_tool"], capture_output=True, text=True
    )
    assert (
        result.returncode == 0
    ), f"Import sorting check failed:\n{result.stdout}\n{result.stderr}"
